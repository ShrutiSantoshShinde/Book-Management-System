...
public class Book_Frame {
    ...
    void getConnectionFromMySQL(){
        try {
            con=DriverManager.getConnection("jdbc:mysql://localhost:3306/book","root","shreya27");
    ...
    void setDataOnTable(ArrayList<Book> bookList){
        ...
        for(int i=0;i<bookList.size();i++){
            obj[i][0]=bookList.get(i).getBook_id();
    ...
    class InsertBookRecord implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            String q="insert into Book (book_id,title,price,author,publisher) values(?,?,?,?,?)";
            try{
                ps =con.prepareStatement(q);
                ps.setInt(1,b1.getBook_id());
        ...
        Book readFromData(){
            Book b1=new Book();
            b1.setBook_id(Integer.parseInt(t1.getText()));
    ...
    class UpdateBookRecord implements ActionListener{
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            String q="update Book set title=?,price=?,author=?,publisher=? where book_id=?";
            try{
                ps=con.prepareStatement(q);
                for(int i=0;i<updatedBookList.size();i++){
                    ...
                    ps.setInt(5,updatedBookList.get(i).getBook_id());
                    ps.executeUpdate();
                }
            }
            catch(SQLException ex){
        ...
        ArrayList<Book>readTableData(){
            ...
            for(int i=0;i<table.getRowCount();i++){
                Book b=new Book();
                b.setBook_id(Integer.parseInt(table.getValueAt(i,0).toString()));
